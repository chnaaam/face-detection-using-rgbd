...
class DepthBackboneLayer(nn.Module):
    def __init__(
            self,
            out_channels=256,
            return_layers={
        ...
        super(DepthBackboneLayer, self).__init__()
...
if __name__ == "__main__":
    sample = torch.ones(size=(1, 1, 640, 640))
    depth_backbone_layer = DepthBackboneLayer(out_channels=512)

    depth_backbone_layer(sample)